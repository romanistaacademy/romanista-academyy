...
الرسالة: ${formData.message}
      ...
      // Open email client directly with mailto
      const mailtoLink = `mailto:ROMANISTAACADEMY@GMAIL.COM?subject=اقتراح جديد من ${encodeURIComponent(formData.name)}&body=${encodeURIComponent(emailBody)}`;
      ...
      // Fallback: open email client anyway
      const mailtoLink = `mailto:ROMANISTAACADEMY@GMAIL.COM?subject=اقتراح جديد من ${encodeURIComponent(formData.name)}&body=${encodeURIComponent(`اسم الشخص: ${formData.name}\\nرقم الهاتف: ${formData.phone}\\nالرسالة: ${formData.message}`)}`;
      window.location.href = mailtoLink;
      setSubmitted(true);
      setFormData({ name: '', phone: '', message: '' });
      setTimeout(() => setSubmitted(false), 5000);
        ...
        >
          {submitted ? (
            ...
            >
              ...
              <p className="text-gray-300">شكراً لك على ملاحظاتك. سنرد عليك قريباً عبر البريد الإلكتروني.</p>
            </motion.div>
          ) : (
